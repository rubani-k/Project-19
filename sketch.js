...
var gorilla, gorillaImg, gorillaGroup;
var gameOver;
var lives;
...
function draw() {
  ...
  if(path.y > 400 ){
    ...
    if (cashG.isTouching(boy)) {
      ...
      if(swordGroup.isTouching(boy) || gorillaGroup.isTouching(boy)) {
        gameState=END;
...
if(World.frameCount % 200 == 0)
  {
    gorilla = createSprite(70,550,20,20);
gorilla.x = Math.round(random(100,300));
gorilla.y = Math.round(random(100,500));
gorilla.addImage(gorillaImg);
gorilla.scale = 0.2;
gorilla.velocityY = -5;
gorilla.lifetime = 150;
